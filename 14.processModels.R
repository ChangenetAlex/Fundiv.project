...
Explain <- c("BA.ha.plot.1","BA.O.plot.1","BAj.plot.1","dbh.plot.mean","BAIj.plot.1.mean","BAIj.plot.1",
             ...
             "treeNbr","yearsbetweensurveys","bio13_climate_mean.30","bio5_climate_mean.30","min_spei12","mean_spei12","Plotcat")
...
Explain <- c("bio13_climate_mean.30","bio5_climate_mean.30","min_spei12","mean_spei12",
...
list.dirs(getwd())
setwd(list.dirs(getwd())[82]) # Chose the model file
list.files()
assign(sub(".rda","",list.files()[13], ignore.case = FALSE,fixed = T),get(load(file = list.files()[13]))) 
rm("x")
summary(get(sub(".rda","",list.files()[16])))
...
#### Coef + valid ########
##########################
Diagnostic(Mbin7A,0.66,F) # Pbm with the coreelog function 
ExtracTest(Mbin7A) # Me donne la liste des paramètres de mon modèles. 
for (i in c(A[c(1:11)])){
  Effect_coef(Mbin7A,i)}     # Para que l'on veut regarder  parmis ceux citer  
Effect_summary(Mbin7A,"biotic") #
ggEffect(Mbin7A,'REL',"sum",band=T)
ggEffect(Mbin7A,'ABS',"sum",band=T) 
...
### Load preexisting models 
list.dirs(getwd())
setwd(list.dirs(getwd())[90]) # Chose the model file
list.files()
assign(sub(".rda","",list.files()[2], ignore.case = FALSE,fixed = T),get(load(file = list.files()[2]))) 
rm("x")
summary(get(sub(".rda","",list.files()[2])))

#Diagnostic(Mbin13A.18,0.66,F)
Extraction(Mbin1B.23)
summary(Mbin1B.23)
ModelBoot(Mbin1B.23,5,11,LvL=30,CAT=CAT,nBoot=1000,Yportion = 0.66,saveboot=T,nCoeur=13)
ModelBoot(Mbin1B.23,10,11,LvL=30,CAT=CAT,nBoot=1000,Yportion = 0.66,saveboot=T,nCoeur=13)
ModelBoot(Mbin1B.23,9,11,LvL=30,CAT=CAT,nBoot=1000,Yportion = 0.66,saveboot=T,nCoeur=13)
...
### Load preexisting models 
list.dirs(getwd())
setwd(list.dirs(getwd())[91]) # Chose the model file
list.files()
...
rm("x")
summary(get(sub(".rda","",list.files()[2])))
#Diagnostic(MnbZT3B.27,0.66,F)
Extraction(MnbZT1B.27)
summary(MnbZT1B.27)
ModelBoot(MnbZT1B.27,8,9,LvL=30,CAT=CAT,nBoot=1000,Yportion = 0.66,saveboot=T,nCoeur=13)
ModelBoot(MnbZT1B.27,6,10,LvL=30,CAT=CAT,nBoot=1000,Yportion = 0.66,saveboot=T,nCoeur=13)
ModelBoot(MnbZT7A.23,8,10,LvL=30,CAT=CAT,nBoot=1000,Yportion = 0.66,saveboot=T,nCoeur=13)
